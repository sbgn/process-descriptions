...
other within a complex because they form the complex. Similarly, \fig{overlap-bad} illustrates the problem using an incorrect map where a compartment hides an EPN.
...
Edges must be drawn on the top of the node (\fig{layout2}). See also recommendation \sect{crosEdNo}.
...
Edges must not overlap the border lines of nodes (\fig{layout3}).
...
Edges must not overlap (\fig{layout4}). This includes touching of edges.
...
\item The arcs linking the square glyph of a \glyph{process} to the \glyph{consumption} and \glyph{production arcs}
 must be attached to the centre of opposite sides (\fig{layout6}).
