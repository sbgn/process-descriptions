...
\begin{figure}[H]
  ...
  \caption{This example of a \PD \add{map} uses two kinds of entity pool nodes: one
...
\begin{table}[bh]
  ...
  \begin{tabular}{@{}llP{2.4in}P{1.6in}@{}}
    ...
    Container node	
    % \rougny{Complexes also belong to EPNs, putting them there is maybe not relevant
    % VT: Yes can be removed. What about 'Container node' Do we have this term somewhere else? It has been changed in the table of content for compartments.
    % UD: The Reference Card uses the term "Container node" but only lists compartment as a container node, complexes are listed under EPNs. Actually I think a complex is both an EPN and a container node but for consistency with the Reference Card, we might want to move complexes to EPN.}
    & CN
    & An encapsulation of one or more other SBGN constructs
    & \corr{Complexes, c}{C}ompartments \\[1.6em]
    ...
    Arc
    & ---
    & Links between EPNs\corr{ or}{,} CNs \add{or Logical Operators} to PNs or \corr{CNs to indicate influences}{Logical operators}
