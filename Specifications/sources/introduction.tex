...
\borlinghaus{This sentence is a bit long. Maybe something like this might make it easier to understand: "Furthermore, investigations of biological systems in synthetic biology, which reconstruct biological systems, are often distributed over different labs and activities. There is a need for an exchange of unambiguous (system?) descriptions, as engineers exchange circuit diagrams."}
...
groups.
\borlinghaus{Maybe we should also have one sentence talking about the available tools. If someone reads about SBGN for the first time they might wonder how they are supposed to draw maps and in which format.\\
AR: I think the specs should be independent of the tools. Available tools (and among them the most practical) should be mentioned on the website and in educational papers.}
...
\rougny{This might not be respected for L1V2? For example, we deprecate some glyph shapes (\eg ellipse to stadium for state variables), making old maps incompatible. Augustin: I say clean up this document and then we figure out what is the correct level/version.
VT: so should it be L2V1.0 then? There is also the concept of subunits that is added now in the spec + 'source and sink' became 'empty set', no?
UD: "Deprecated" doesn't necessarily mean "not supported any longer at all". It often means in software that this should be avoided but still supported.}
