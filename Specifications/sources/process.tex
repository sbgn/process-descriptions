...
A \corr{process}{\glyph{process} represents a generic process that} transforms a set of entity pools (represented by \glyph{EPNs} in \SBGNPDLone) into another set of entity pools.
...
\rougny{Replace Origin by Incoming (arcs)? (see issue \#204)
Vasundra: Incoming and Outgoing seems more intuitive for me and I would be in favor for this.
UD: we can use incoming / outgoing I think or source / target. I would also change "one or several" with "one or more" or "at least one". This applies to all/most process glyphs I suppose.
AR: I would say source and target are rather for arcs, not nodes.
Also, it is in red, so it was supposed to be suppressed.
Should we keep it? But then we would have to add these elements for every node I guess (EPNs, compartments, \dots).
}
...
\rougny{Processes are nodes. As such, I do not think they should have origin and target entries, that are intuitively related to arcs.}

\glyphContainer
A \glyph{process} is represented by a square shape.
The shape is linked to two ports, that are small arcs attached to the centres of opposite sides of the shape, as shown in \fig{process}.
...
\glyphLabel
\corr{A \glyph{process} is not identified by any label}{None}.

\glyphAux
\corr{A \glyph{process} does not carry any auxiliary items}{None}.
...
A process is regarded as reversible if both \corr{`sides'}{ports} of the process are connected to \glyph{production} arcs (see section \ref{sec: semantics reversible procs}).
