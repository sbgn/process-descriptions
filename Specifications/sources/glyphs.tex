...
In doing so, they change the meaning of the glyph \corr{and/}{}or provide additional information about it.
These can provide \add{specific} annotation (\glyph{unit of information}), state information (\glyph{state variable}), indicate duplication of entity pool nodes (\glyph{clone marker}), describe specific glyphs (\glyph{subunit} for \glyph{complex})\add{, or provide handles to elements lying outside of the maps (\glyph{submap terminal} for \glyph{submap})}.

\input{sources/unitInformation.tex}
\input{sources/stateVariable.tex} % FIX 101717
\input{sources/cloneMarker.tex} % Done, Ugur
\input{sources/subunit.tex}
\input{sources/submapTerminal.tex}
...
In addition to the material entities, \PD represents two conceptual \corr{entities}{entity pools}: \glyph{empty set} (\sect{emptySet}), and \glyph{perturbing agent} (\sect{perturbing agent}).
...
\input{sources/simple_chemical.tex}
\input{sources/macromolecule.tex} % FIX 101717
\input{sources/nucleic_acid_feature.tex}
\input{sources/multimer.tex}
\input{sources/complex.tex}

\input{sources/empty_set.tex}
...
\add{Processes transform entity pools into other entity pools (\sect{PNs}).
...
\add{Modulation arcs represent influences of entity pools on processes.
...
\dogrusoz{Any particular reason we have "nodes" here? Sounded like unnecessary introduction of a term.\\AR: to be consistent with the naming of other classes of nodes: Entity Pool Nodes and Process nodes.\\
AR: actually you're right, removed nodes}
\label{sec:logic}

\add{The \glyph{logical operator} performs an operation on one or more inputs to give a unique output.
...
\rougny{There was no text in V1.3, adapted part of the text of oldV2 to include the equivalence operator. Is the part on the ``union of pools'' exact? Rather talk of subclasses and superclass, or something like that: combines subclasses into a superclass?}

\input{sources/and.tex}
\input{sources/or.tex}
\input{sources/not.tex}
\input{sources/equivalence.tex}
...
\corr{or as an auxilary unit on the \glyph{submap}. The \glyph{clone marker} can also provide additional reference mechanisms and is discussed below (\sect{cloneMarker}).}{}
