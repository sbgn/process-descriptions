...
\begin{valrules}
\defrule{PD1}{A map is valid if it is empty (although not very
  useful).}
\defrule{PD2}{All instances of \classref{SBGNGlyph} must be unique (see
...
\begin{valrules}
\defrule{PD3}{The set of \sbgnclass{SBGNNode}s linked to this node via a
...
\begin{valrules}
\defrule{PD4}{If \attrib{cardinality} $>1$ then the descendant glyph must be displayed
  as a multimer.}
\defrule{PD5}{If the EPN is drawn directly on a \glyph{Map} then
  \attrib{owning\_compartment} is not set. We interpret this as
  belonging to an invisible default compartment.}
\defrule{PD6}{\attrib{natures} can only use the material type
...
\defrule{PD7}{The appropriate subclass of \sbgnclass{CloneMarker} must be used
  to distinguish logically identical instances of this class.}
\defrule{PD8}{All \sbgnclass{StateVariableDefinition}s associated with the
...
\begin{valrules}
\defrule{PD9}{All instances of \glyph{Empty Set} can be regarded as identical
...
\begin{figure}[htb]
  ...
  \caption{Example of \glyph{annotations} adding information to the
    ...
    consistent scheme whithin a map.}
