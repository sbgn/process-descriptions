...
holds all the glyphs (\classref{SBGNGlyph}) drawn in a \PDm. The
\attrib{elements} held should be logically unique and conform to the
...
\begin{valrules}
\defrule{PD01}{A map is valid if it is empty (although not very
  useful).}
\defrule{PD02}{All instances of \classref{SBGNGlyph} must be unique (see
...
\begin{valrules}
\defrule{PD03}{The set of \sbgnclass{SBGNNode}s linked to this node via a
...
\begin{valrules}
\defrule{PD68}{All instances of \sbgnclass{EntityPoolNode} associated with a
  particular \sbgnclass{EntityType} must be of the same class.}
\defrule{PD69}{If an instance of \sbgnclass{EntityType} contains one or more
...
% \newglossaryentry{Rule002}
% {
%     name=Rule002,
%     description={If an instance of \sbgnclass{EntityType} contains one or more
%   instances of \sbgnclass{StateVariableDefinition} then the
%   \sbgnclass{EntityPoolNode}s associated with it must be subclasses of
%   \sbgnclass{StatefulEPN}.}
% }
% \item[\gls{Rule002}] If an instance of \sbgnclass{EntityType} contains one or more
%   instances of \sbgnclass{StateVariableDefinition} then the
%   \sbgnclass{EntityPoolNode}s associated with it must be subclasses of
%   \sbgnclass{StatefulEPN}.
...
\umlclasshead{StateVariableDefinition}
...
\begin{valrules}
\defrule{PD04}{If \attrib{cardinality} $>1$ then the descendant glyph must be displayed
  as a multimer.}
\defrule{PD05}{If the EPN is drawn directly on a \glyph{Map} then
  \attrib{owning\_compartment} is not set. We interpret this as
  belonging to an invisible default compartment.}
\defrule{PD06}{\attrib{natures} can only use the material type
   (section~\ref{sec:techref:material-types-cv}), conceptual type
   (section \ref{sec:techref:conceptual-types-cv}) or physical 
 characteristics (section \ref{sec:techref:physical-characteristics-cv}) controlled vocabularies.}
\defrule{PD07}{The appropriate subclass of \sbgnclass{CloneMarker} must be used
  to distinguish logically identical instances of this class.}
\defrule{PD08}{All \sbgnclass{StateVariableDefinition}s associated with the
...
%   \attrib{owning\_compartment} is not set. We interpret this as
%   belonging to an invisible default compartment.
%  \item \attrib{natures} can only use the material type
%    (section~\ref{sec:techref:material-types-cv}), conceptual type
%    (section \ref{sec:techref:conceptual-types-cv}) or physical 
%  characteristics (section \ref{sec:techref:physical-characteristics-cv}) controlled vocabularies.
...
\begin{valrules}
\defrule{PD09}{All instances of \glyph{Empty Set} can be regarded as identical
therefore no special decoration is used to indicate replication on
the map.}
\defrule{PD10}{The \sbgnclass{EmptySet} must be associated with at least
...
\begin{valrules}
\defrule{PD11}{If a clone marker is used it must be of type \sbgnclass{SimpleCloneMarker}.}
\end{valrules}
...
\umlclasshead{UnspecifiedEntity}
...
\begin{valrules}
\defrule{PD12}{The \sbgnclass{UnspecifiedEntity} cannot have cardinality $>
...
\begin{valrules}
\defrule{PD13}{The \sbgnclass{PerturbingAgent} cannot have \attrib{cardinality} $>
  1$. This means there is no multimer glyph.}
\end{valrules}
...
\begin{valrules}
\defrule{PD14}{State variables do not need to be logically unique, therefore
  two or more state variables with the same name are permitted.}
\defrule{PD15}{The \sbgnclass{LabelledCloneMarker} must be used to indicate
...
\fig{techref:example-glur} represents the glutamate receptor in the open state, with both phosphorylation and glycosylation. The entity carries two functional domains, the ligand-binding domain and the ion pore, and its chemical nature is preceded.
...
\umlclasshead{NucleicAcidFeature}
...
\umlclasshead{Complex}\label{sec:techref:complex}
\label{defn:Complex}
...
\begin{valrules}
\defrule{PD16}{Once a set of subunits are defined for an \sbgnclass{Complex}
  with a given \sbgnclass{EntityType}, then they must be used by every
  instance using that entity type.\query{New rule.}.}
\defrule{PD17}{The set of subunits in the Complex does not identify it. One or
...
\paragraph{Glyph: \glyph{Complex Monomer}}
...
\paragraph{Glyph: \glyph{Complex Multimer}}
...
\begin{valrules}
\defrule{PD18}{The subunits inside the complex must not overlap.}
\defrule{PD19}{The subunits should sit above the clone marker so that they are
  not obscured by it.}
\defrule{PD20}{The label should not be obscured by subunits or obscure them.}
\end{valrules}
...
\begin{valrules}
\defrule{PD21}{Two or more state variables with the same name are
  permitted.}
\defrule{PD22}{State variables with no name set are permitted.}
\defrule{PD23}{Subunits can also contain subunits. There is no limit on such nesting. The namespace rules
  below apply.}
\defrule{PD24}{The subunit defines a namespace for its state variables, e.g.\,
  subunit ``A'' assigned a state variable ``P@Ser202''  and a subunit
  ``B'' assigned the same state variable can be distinguished as
  A:P@Ser202 and B:P@Ser202.}
\defrule{PD25}{If the subunit is of type Complex then \attrib{children} can contain one or
  more \sbgnclass{Subunit} instances.}
\defrule{PD26}{If the subunit has a cardinality $>1$ then this should be
  displayed by the \classref{AttributeValue}.}
\defrule{PD27}{If \attrib{natures} contains one or more instances then these
...
\umlclasshead{ProcessNode}\label{sec:techref:PNs}
...
\umlclasshead{NonStoichiometricProcess}
...
\begin{valrules}
\defrule{PD28}{The number of \attrib{in\_arcs} must be $>0$.}
\defrule{PD29}{\attrib{in\_arc} can only contain instances of
\classref{ModulationArc} and its subclasses.}
\defrule{PD30}{\attrib{out\_arcs} must be empty.}
\end{valrules}
...
The \sbgnclass{Submap} (figure \ref{fig:techref:submapnodeuml})
represents a process or set of processes and is a mechanism for hiding
the detail of a portion of a \PDm. The \sbgnclass{Submap} can be
...
also in the \sbgnclass{Submap}.  It is anticipated that a software tool
will use these forms as a convenient way to hide and show portions of
a large SBGN map.

% is a placeholder for another process
% and is used when one wishes to hide the detail of this process from
% the \PDm. The \sbgnclass{Submap} is not equivalent to an
% \sbgnclass{OmittedProcess} (section \ref{sec:techref:omitted}),
% because it does not necessarily have inputs and outputs. The
% \sbgnclass{Submap} allows the detail of the section of the \PDm to be
% exported to another \PDm and replaced by the \sbgnclass{Submap},
% which acts as a place-holder.  This is described in section
% \ref{sec:techref:map} and the semantics of submap linking is defined
% in section \ref{sec:techref:submaplinkingsemantics}.

\begin{figure}[htb]
  \centering
  \includegraphics[width = \textwidth]{images/submapnodeuml}
  \caption{The UML definition of the \sbgnclass{Submap} and its
...
\begin{attributes}
\associtem{terminals}{SubmapTerminal}{1..*} The terminals show how the
...
\begin{valrules}
\defrule{PD31}{All instances of \classref{SubmapTerminal} held by this class
  must be logically unique.}
\defrule{PD32}{\attrib{in\_arcs} and \attrib{out\_arcs} must be empty (\ie
  degree $=0$).}
\end{valrules}
...
\begin{figure}[htb]
  ...
  \caption{The \PD glyph for \glyph{submap}. (a) the basic glyph
    ...
    containing \glyph{submap terminals}. (c) An expanded view of the
...
\begin{valrules}
\defrule{PD33}{\attrib{in\_arc} can only contain one or more instances of
  \sbgnclass{LogicArc}.}
\defrule{PD34}{\attrib{out\_arc} can only contain one or more instances of
  \sbgnclass{LogicArc} or \sbgnclass{ModulationArc}.}
\defrule{PD35}{if \attrib{operator\_type} is AND or OR, then \attrib{in\_arc} must
  contain two or more arcs.}
\defrule{PD36}{if \attrib{operator\_type} is NOT then \attrib{in\_arc} must
  contain only one arc.}
\defrule{PD37}{\attrib{out\_arc} can contain only one arc.}
\end{valrules}
...
\umlclasshead{RelationalOperator}
\label{sec:techref:relation}
\label{defn:RelationalOperator}

\begin{figure}[htb]
  \centering
  \includegraphics[width = 0.75\textwidth]{images/relationaloperatoruml}
  \caption{The UML definition of the \sbgnclass{RelationalOperator}.}
...
To enable the representation of EPNs together with corresponding sub-classes of this EPNs, we introduce the equivalence operator glyph. Please note that this glyph can potentially introduce ambiguity. Examples of the correct use of the glyph are provided in Appendix D together with examples of misuse and with the proposed set of rules suggested as guideline for the use of the equivalence operator glyph.
 ...
 The \sbgnclass{RelationalOperator} (figure \ref{fig:techref:relationaloperatoruml} performs a Boolean operation on one or
more inputs to give a binary output. The input must be a Boolean
value, and are obtained from the \classref{LogicArc} connected to the
\sbgnclass{LogicOperator}. The output a two-value quantity,  0 for False and positive
non-zero for True. This is required because the output of the
...
\begin{valrules}
\defrule{PD??}{\attrib{in\_arc} can only contain one or more instances of
  \sbgnclass{EquivalenceArc}.}
\defrule{PD??}{\attrib{out\_arc} can only contain one or more instances of
  \sbgnclass{LogicArc} or \sbgnclass{ModulationArc}.}
\end{valrules}
...
\paragraph{Glyph: \glyph{Equivalence}}\label{sec:techref:equivalence}

\begin{glyphDescription}
 \glyphSboTerm SBO:0000392 equivalence (is a).
 \glyphNode \glyph{Equivalence} is represented by a circle carrying uppercase letter "xi" (Ξ) of the Greek alphabet. The choice of the symbol is motivated by its use in mathematics for describing relationships as "equivalent to", "identical to". 
\end{glyphDescription}
...
\umlclasshead{StoichiometricProcess}
...
\begin{valrules}
\defrule{PD38}{The \attrib{in\_arc} must contain one or more
  \sbgnclass{FluxArc}s containing the same \attrib{flux\_type} value.}
\defrule{PD39}{The \attrib{in\_arc} may only contains \sbgnclass{FluxArc}
  instances with a \attrib{flux\_type} of `consumption', or `reversible'.}
\defrule{PD40}{The \attrib{in\_arc} may only contains \sbgnclass{FluxArc}
  instances with a \attrib{flux\_type} of `consumption', or `reversible'.}
\defrule{PD41}{In addition the \attrib{in\_arc} may contain zero, one or more
  instances of \sbgnclass{ModulationArc}.}
\defrule{PD42}{The \attrib{out\_arc} must contain one or more
 instances of \sbgnclass{FluxArc} with a \attrib{flux\_type} or `production'.}
\defrule{PD43}{If \attrib{in\_arcs}  contains one or more \sbgnclass{FluxArc}s
  of type `reversible' this process reversible.}
\defrule{PD44}{The \sbgnclass{EntityPoolNode}s that make up the LHS of the
...
\defrule{PD45}{If at least one \sbgnclass{FluxArc} associated with a
  ...
  \glyph{stoichiometry label} then all must.\query{Take from previous
    ...
    is too restrictive.}}
\defrule{PD46}{If more than one set of stoichiometries can be applied to the
...
\begin{valrules}
\defrule{PD47}{The process must be irreversible.}
\defrule{PD48}{There can only be one `production' \sbgnclass{FluxArc}, with
  \attrib{stoichiometry} $= 1$.}
\defrule{PD49}{If a \sbgnclass{Complex} is on the RHS of the association then
...
\begin{valrules}
\defrule{PD50}{The process must be irreversible.}
\defrule{PD51}{There can only be one `consumption \sbgnclass{FluxArc}, with
  \attrib{stoichiometry} $= 1$.}
\defrule{PD52}{If a \sbgnclass{Complex} is on the LHS of the dissociation then
...
\begin{valrules}
\defrule{PD53}{The process must be irreversible.}
\defrule{PD54}{\attrib{epns} must contain a unique set of
...
\glyphLabel An \glyph{annotation} contains information placed in an
...
\glyph{tag}, which can be used in a map referred to by a \glyph{submap}
(\sect{techref:submap}) or as an auxiliary unit on the \glyph{submap}. The
...
\begin{valrules}
\defrule{PD55}{Two or more instances of \sbgnclass{CrossReference} with the
...
\classref{Submap}. It provides the reference that is the link to a
tag in the submap that the \sbgnclass{Submap} refers to.
...
\begin{valrules}
\defrule{PD56}{All values of \attrib{reference\_id} must be unique within an
...
\begin{valrules}
\defrule{PD57}{if the \attrib{stoichiometry} is not displayed then it is
...
\defrule{PD58}{If the \attrib{stoichiometry} $>1$ then the stoichiometry must
  be displayed.\query{See above comment.}}
\defrule{PD59}{if \attrib{flux\_type} $=$ `consumption' or 'reversible' then:
  \begin{itemize}
  ...
  \end{itemize}}
\defrule{PD60}{if \attrib{flux\_type} $=$ `production' then:
  \begin{itemize}
...
\begin{valrules}
\defrule{PD61}{At most one \glyph{necessary stimulation} can be assigned to a process node. Two \glyph{necessary stimulations}
...
\defrule{PD62}{At most one \glyph{catalysis} can be assigned to a
...
\begin{valrules}
\defrule{PD63}{The \attrib{in\_node} must be an instance of
  \sbgnclass{EntityPoolNode} or \sbgnclass{LogicalOperator}.}
\defrule{PD64}{The \attrib{out\_node} must be an instance of
...
to an \classref{EntityPoolNode} in another \PDm (represented by \classref{CrossReference})
with an \classref{EntityPoolNode} in this map.
...
\umlclasshead{SimpleCloneMarker}
...
\begin{valrules}
\defrule{PD65}{Only subclasses of \classref{StatelessEPN} can contain
...
\umlclasshead{LabelledClonerMarker}
...
\begin{valrules}
\defrule{PD66}{At least two or more instanced of a
  \sbgnclass{LabelledCloneMarker} with the same \attrib{label} must
  exist in this same in a given \classref{Map}.}
\defrule{PD67}{Only subclasses of \classref{StatefulEPN} can contain
...
defined in this specification can be used in a valid \PDm\query{The
  previous version was a bit vague but implied that the CVs were
...
% \classref{Submap}, which acts as a place-holder. In the case of an
% SBGN description that is made available through a software tool, the
% content of a \sbgnclass{Submap} may be available to the tool.  A
...
% than those applying to the \sbgnclass{Submap} so we will detail
...
% %The \glyph{submap node} carries five terminals, four linked to EPNs and one linked
% %to a \glyph{compartment}.  The latter is particularly important in the case of EPNs
% %present only in a \glyph{compartment} enclosed in a \glyph{submap node}, and that
% %are not linked to terminals themselves.
% The \glyph{submap} carries four terminals linked to EPNs.
...
% in the previous version of the spec  included compartment
