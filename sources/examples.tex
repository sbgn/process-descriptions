\chapter{Complete examples of \SBGNPDLone graphs}
...
\fig{glycolysis} presents an example of metabolic pathway, that examplifies the use of the \glyph{EPNs} \glyph{simple chemical}, \glyph{macromolecule}, and \glyph{clone marker}, the \glyph{PNs} \glyph{transition}, and the \glyph{connecting arcs} \glyph{consumption}, \glyph{production} and \glyph{catalysis}.
...
\fig{mapk} is an expanded version of the submap present on the diagram present in \fig{insulin}. It shows the use of \glyph{tag}.
...
\fig{muscle} introduces an SBGN \PD that spans several compartments. Note that the compartment ``synaptic vesicle'' is not \textbf{contained} in the compartment ``synaptic button'' but \textbf{overlap} it. The \glyph{simple chemical} ``ACh'' of the ''synaptic vesicle`` is not the same \glyph{EPN} than the ``ACh'' of the ``synaptic button'' and of ``synaptic cleft''.  The situation is simular with the compartments ``ER'' and ``muscle cytosol''.  The diagram exemplifies the use of of the \glyph{PN} \glyph{omitted} and \glyph{dissociation}, and the \glyph{connecting arc} \glyph{trigger}.
...
\fig{IFN} introduces the use of \SBGNPDLone to encode gene regulatory networks. It also show the use of the \glyph{EPNs} \glyph{Source} and the \glyph{logical operator} \glyph{and}.
