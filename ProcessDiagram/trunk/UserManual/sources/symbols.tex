...
% Refactor the structure of this chapter. We need less sections and a simpler structure. For instance, auxiliary units must be defined in the EPN section. 

An SBGN \PDm is mainly is bipartite graph, i.e. it is made up of two types of nodes that connect in an alternate way (some exceptions are described below, e.g. when \glyph{logical operators} or \glyph{tag} are used). The nodes are the \glyph{entity pools nodes}, representing the things that are modified by processes and the \glyph{process nodes}. The two types of nodes are connected by arcs. In addition, the \glyph{entity pools nodes} can be contained in \glyph{compartments}. 
...
% 
% \section{Modification of the entity pool nodes}
