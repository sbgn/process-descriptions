...
\hline\hline
  Namespace scope & Entities affected & Notes\\\hline\hline}
\tabletail{\hline
...
A \glyph{states variable} may have a name. If the
...
An \glyph{entity pool node} intrisicaly possesses a set of \glyph{state variables}, which is the union
of all the \glyph{state variables} displayed on all the related \glyph{entity pool nodes} in the map. 
%(see map \ref{fig: sbgn}).  
All stateful \glyph{EPNs} must explicitly display the same set of
\glyph{state variables} and each \glyph{state variable} must be assigned a value, or be
assigned the value ``Not Set''.

\subsection{Compartment spanning}

An \glyph{EPN} cannot \emph{belong} to more than one
\glyph{compartment}. However, an \glyph{EPN} can be \emph{drawn} over more than one
...
author. ComplexNodes may contain \glyph{EPNs} which belong to different
...
which it converts its input \glyph{EPNs} to its output \glyph{EPNs}. This concept is
...
\item At most one \glyph{necessary stimulation} can be assigned to a \glyph{process}. Two \glyph{necessary stimulations}
...
\item At most one \glyph{catalysis} can be assigned to a \glyph{process}. A catalysis
  ...
  be assigned to the same \glyph{process} as they represent
  independent reactions. Other \glyph{EPNs} can be
...
\glyph{Process nodes} are deemed to be reversible if they have \glyph{production} arcs on both ``sides'' of the \glyph{process}. A mixture of \glyph{consumption} and \glyph{production} arcs on the same side of a \glyph{process} is not permitted. Semantically, the \glyph{production} arc can be thought of as allowing a reversible flow of entities between the \glyph{process} and the \glyph{EPN}. A \glyph{consumption} arc only permits an irreversible flow from the \glyph{EPN} to the \glyph{process}. In this way the \glyph{consumption} arc forces the \glyph{process} to be irreversible. \glyph{Consumption} arcs cannot be associated with both sides of a \glyph{process} as this would prohibit any flow through the \glyph{process}.

The semantics of \glyph{modulation} is the same as for irreversible processes, .i.e. the amount of entity in the modulation pool affects the rate of the process.
...
a submap into the main map by eliminating the \glyph{submaps} and merging
the equivalent \glyph{entity pool nodes} in both maps.

\subsubsection{Rules for mapping to submaps}

An \glyph{entity pool node} in the main map can be mapped to one in the \glyph{submap}
using a \glyph{tag} in the \glyph{submap} and a submap terminal (see \sect{submap}) in the main map. For a
mapping between map and submap to exists the following must be true:

\begin{enumerate}
\item The identifiers in the \glyph{tag} and the submap terminal must be identical.
\item The \glyph{entity pool nodes} must be identical.
\end{enumerate}

% \subsubsection{Requirement to define a mapping}
% 
% If a map and a submap both contain the same EntityPoolNode, then a
% mapping between them must be defined as above.
%% NLN: This is identical to above

\subsubsection{Cloning consistency}

If an \glyph{entity pool node} is cloned in the main map, then it must also be
marked as cloned in the \glyph{submap} even if there is only one copy of the \glyph{entity pool node}
in the \glyph{submap}. The converse rule also applies where the
\glyph{entity pool node} is cloned in the \glyph{submap}, but not the main map.
