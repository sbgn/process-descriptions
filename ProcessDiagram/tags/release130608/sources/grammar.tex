...
such as whether duplicate symbols are permitted. In addition, we must define what the notation ``means'' --- how does it represent a biological pathway? This is semantcs, and it is essential if a reader is to understand a SBGN diagram without external help, and a writer is to create one that reflects his understanding of a biologiocal system.
...
The \PD{} in SBGN describes biological processes involving biological entities. An Entity Pool Node (section \ref{sec:EPNs}) such as a molecule is converted into other Entity Pool Nodes via a process. 

It may be convenient to think of an SBGN \PD{} as describing the
flow of a fluid. In this analogy each Entity Pool Node represents a
tank of fluid which may be emptied via a pipe (the consumption arc, section \ref{sec:consumption}) 
that is connected to a valve (the process node, section \ref{sec:PNs}), which in turn is
connected to another pipe (the production arc, section \ref{sec:production}) that fills another tank
(Entity Pool Node). A valve can be
...
more external sources (Source, section \ref{sec:sourceSink}) and somewhere for it to drain away (the
Sink).
...
In order to formalise the conceptual representation described above
and the semantic rules we will describe later we have developed a UML
...
(figure~\ref{fig:sbgn_visual}) directly maps to the glyphs used to
...
(figure~\ref{fig:sbgn_conceptual}). It is important in defining
concepts such as Entity types and state variables that are implicit in the Visual Layer.

 \begin{landscape}
\begin{figure}[p]
\begin{center}
\includegraphics[width=0.9\linewidth]{images/sbgn_visual}
\caption{UML model of SBGN visual representation. Class names shown in italics are Abstract classes and do not correspond to a Glyph. Classes in non-italicised script with the suffixes ``Node'' and ``Arc'' correspond to node and arc glyphs repectively.}
...
\begin{figure}[p]
\begin{center}
\includegraphics[width=\linewidth]{images/sbgn_conceptual}
\caption{UML model of SBGN conceptual representation. The main difference here is that the model is non-redundant with only one instance of an EntityPool with a given identity. Each class has a mapping back to the Visual Layer.}
...
defining the rules for duplication and the module interface later.
...
\begin{supertabular}{|p{3cm}|p{4cm}|p{6cm}|p{3cm}|p{\commlen}|}\hline
%
MapDiagram & name (I) & Name of the map. & N/A & The diagram is the visualisation of an SBGN map.\\\hline
%
SubmapDiagram & mainMapName (I) & Name of the main map this this is a sub-map of.& N/A & \multirow{2}{\commlen}{An SBGN diagram used to define an SBGN module.}\\
...
%
ModulatableNode & modulatingNodes (I) \{0 \ldots *\} & Any number of nodes that modulate this one.& Abstract & Nodes that can be modulated and so are linked to the end of a modulation arc.\\\hline
%
ModulatingNode & & & Abstract & Nodes that can modulate modulateable nodes. They are linked to the start of a modulating arc.\\\hline
%
...
%
ObservableNode & name (I) & Name of the phenotype. & Observable & \\\hline
%
PerturbationNode & name (I) & Name of the perturbation. & Perturbation & \\\hline
%
MacromoleculeNode & As StatefulEntityPoolNode & & MacromoleculeNode & \multirow{2}{\commlen}{} \\
 & macromoleculeType (I) & The type of the macromolecule. Currently there are no constraints on permitted types. & & \\\hline
%
GeneticEntity\-Node & As StatefulEntityPoolNode & & GeneticEntity & \\\hline
%
...
%
Mulitmer\-Macromolecule\-Node & As MacromoleculeNode & & Multimer\-Macromolecule & \multirow{2}{\commlen}{Note that all sub-units have the \emph{same} StateDescription values.} \\
...
%
MultimerComplex\-Node & As ComplexNode & & MultimerComplex & \multirow{2}{\commlen}{}\\
...
%
StateDescription & owningStatefulEPN & Identity of the owning StatefulEntityPoolNode (not displayed, but implied by ``ownership'' of the glyph). & State Variables & \multirow{3}{\commlen}{It is important to note that the StateDescription always defines a \emph{set} of values.} \\
 & variable & The name of the ``state variable'' associated a StatefulEntityPoolNode. & & \\
 & value &  The definition of a set of values for the given variable. & & \\\hline
%
...
LogicGateNode & gateType (I) & The type of boolean gate. Currently one of OR, AND, NOT. & Abstract & \multirow{3}{\commlen}{Not a glyph but a placeholder for the common behaviour of all logic gates.}\\
 ...
 & modulatedNode (I) & A ModulatableNode connected via a ModulationArc or another LogicGateNode connected via a LogicArc. & & \\\hline
%
...
%
SubMapTagNode & subMapName (I) & Name of submap that owns submap tag. & SubMapTag & \\
& identifier (I) & A name that uniquely identifies the tag on the submap. & & \\\hline
%
...
%
ModulationArc & modulatingNode (I) & The ModulatingNode that is the source of this link. & Modulation & \multirow{2}{\commlen}{} \\
...
%
TriggerArc & As ModulationArc & & Inhibition & \\\hline
%
LogicArc & modulatingNode (I) & The ModulatingNode that is the source of this link. & LogicArc & \multirow{2}{\commlen}{Conceptually this class can be thought of as converting a continuous input (the population of the EntityPool) into a discrete Boolean output (True or False).} \\
...
EquivalenceArc & subMapTag (I) & The tag assigned to with the EPN. & Tag & \multirow{2}{\commlen}{} \\
 & EPN (I) & The EPN assigned to the submap tag. & & \\\hline
%
\end{supertabular}
\end{scriptsize}
\end{center}
...
%
Compartment & name (I) & The name of the compartment. & & CompartmentShape\\
...
& type (I) &  Type of the Entity. One of: SimpleChemical, UnspeciedEntity, Complex, Macromolecule, and GeneticEntity. & & \\
 & annotations \{0 \ldots *\} & Annotation must be associated with the type and be general to all Entity Pool Node. If it were associated with the Sprecies then this would be synonomous with a state description. & & \\\hline
%
...
%
Macromolecule\-Entity\-Type & macromoleculeType (I) &  The type of the macromolecule. The permitted type names are not constrained as yet. Suggest types are protein, mRNA, miRNA, tRNA, rRNA, DNA, polysaccaride. & & Implicitly defined by a MacromoleculeNode and MultimerMacromoleculeNode\\\hline
%
StateVariable & name (I) & The name of the state variable. & \multirow{2}{\desclen}{The state variable is assigned to a StatefulEntityType and can have one or more values assigned to it when used in a StatefulEntityPool.} & \multirow{2}{\mappinglen}{The name of a StateDescription.} \\
 & domainValues \{1 \ldots * \} & The domain of the StateVariable, i.e. the set of possible values it can hold. This is required because an Stateful\-Entity\-Pool is assigned a set of state values for a given StateVariable. & & \\\hline
%
StateValue & value (I) & The value of the state. & A value that can be assigned to a state variable. & The value string or ``regexp'' used in the StateDescription.\\\hline
%
...
StatefulEntity\-Pool & As EntityPool & & & \multirow{2}{\mappinglen}{Stateful\-Entity\-Pool\-Node} \\
 & stateValueSets \{1 \ldots * \} &  A set of state values assigned to a the StateVariables of the associated StatefulEntityType. & & \\\hline
%
ComplexEntityPool & As StatefulEntity & & \multirow{2}{\desclen}{The instantiation of a complex that is defined by it's own state and the states of its subunits.} & ComplexNode \\
 & subunitValues (I) \{0 \ldots * \} &  The set of StateValues for all subunits. & & \\\hline
%
...
%
Process & processType (I) &  One of: Unspecified, Dissociation, Association, Omitted Process, Uncertain Process. & & The type is specified by the type of Glyph used: ProcessNode, DossociationNode, AssociationNode, OmittedProcessNode, UndertainProcessNode.\\\hline
%
Output & process (I) & The process the is the output of. & & ProductionArc \\
 & product (I) & The Entity Pool Node that is the product of the process. & & \\\hline
%
Input & substrate (I) & The Entity Pool Node that is the input to the process. & & ConsumptionArc \\
 & process (I) & The process that this is the input of. & & ConsumptionArc \\\hline
%
Control & type (R) & The type of the control. One of Modulation, Stimulation, Inhibition, Trigger, Catalysis or LogicGate. & & ModulationArc \\\hline
%
...
%
Observable & name & Name of the phenotype. & & ObservableNode \\\hline
%
...
%
SubMap\-Entity\-Mapping & identifier (I) & Identifier in Visual layer to link Entity\-Pool\-Nodes in the main map and submap. & \multirow{3}{\desclen}{A definition of the mapping between the submap and main map for a single EntityPool.} & \multirow{3}{\mappinglen}{SubMap\-Tag\-Node and Tag\-Node}\\
